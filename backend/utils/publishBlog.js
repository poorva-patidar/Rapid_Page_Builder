...
async function job() {
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    if (
      ...
      const page = await Page.findByIdAndUpdate(pages[i]._id , { emailSent: true } , {
        new: true,
        runValidators: true,
      });
            ...
            <body>
              <div class="container">
                <div class="d-flex flex-column justify-content-center text-justify border rounded-2 p-4 mt-3">
                  ...
                  <div>Publish Time: ${new Date(page.publishTime).toLocaleDateString("en-US", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                  }) + " " + new Date(page.publishTime).toLocaleTimeString("en-US")}</div>
                </div>
              </div>
            </body>
          </html>`,
